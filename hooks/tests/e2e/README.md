...
**決定 (2026-06-10)**: CC 既定 = `standard`。根拠は「CC には hooks の floors があり、invariant + floors が二重に効くため既定手順を緩めても worst-case を防げる」。実装は `session-context.sh` の `HIKIZAN_TIER` 既定値 (= `standard`) と `templates/CLAUDE.md` tier 節に固定済み。E2E ベンチの定量確認 (overhead < +15%) は未実測のため、回したら本節に数値を追記する。`guided` にしたい環境は `HIKIZAN_TIER=guided` で opt-in。
