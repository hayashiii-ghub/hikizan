...
assert_eq "Cursor manifest has no startup hook" 'false' \
  "$(jq 'has("hooks")' "$ROOT/.cursor-plugin/plugin.json")"
...
file_status=0
[ -f "$PI_ADAPTER" ] || file_status=1
assert_exit "pi adapter exists" 0 "$file_status"
...
file_status=0
[ -f "$EXA_SEARCH" ] || file_status=1
assert_exit "pi Exa tool exists" 0 "$file_status"
file_status=0
[ -f "$EXA_CLIENT" ] || file_status=1
assert_exit "pi Exa client exists" 0 "$file_status"
