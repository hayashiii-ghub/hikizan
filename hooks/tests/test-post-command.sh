...
# run_post <command> [cwd] [destructive decision] -> sets HZ_CODE and METRICS_FILE
run_post() {
  local cmd="$1" cwd="${2:-/tmp}" destructive_decision="${3:-ask}"
  ...
  printf '%s' "$payload" | bash "$HOOK" "$destructive_decision" >/dev/null 2>/dev/null
  HZ_CODE=$?
}
...
run_post "rm -rf /tmp/x" /tmp block
assert_eq "rm -rf: deny-only harness decision" "block" "$(jq -r '.decision' "$METRICS_FILE")"
...
# 6. segment-local flags: a later command's --draft cannot exempt the PR.
run_post "gh pr create --title x&&echo --draft"
assert_eq "compound PR: exit 0" "0" "$HZ_CODE"
assert_eq "compound PR: 1 metrics line" "1" "$(metrics_lines)"
assert_eq "compound PR: condition" "no_draft_no_reviewer" "$(jq -r '.condition' "$METRICS_FILE")"
