...
フックは次の処理を扱います。piだけは、利用者が`EXA_API_KEY`を設定した場合の任意Web検索と、本番・公開環境へ影響しやすいbashコマンドの実行前確認も提供します。実装判断、コミット可否、会話回数による内省、利用状況計測は扱いません。
...
- `adapters/pi/index.ts`：piの起動規則、Git状態、TUI、画面検証、任意Web検索の入口
