...
1. **レール・opt-out・floors**：skill 本文は弱いモデル基準のレール (番号付き手順 + 穴埋めテンプレ)。standard tier (hooks=floors のある環境) には opt-out 前文 (`context/standard-preamble.md`) を注入し「手順は自由、出口は固定」にする。hooks は tier に関わらず効く決定論的な下限。自律度はモデル検知でなく、環境構築時にどこまで仕組みを用意したかで決まる。
