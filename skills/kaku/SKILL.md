...
3. `references/writing-style.md` の規範に従って書く。段落は一トピック、一文一行
4. 書き上げたら `references/writing-style.md` の各節の点検項目を上から当て、外れた箇所だけ直す
5. 下の「報告」を埋めて返す

## 手順 (推敲)

1. 対象の文と、直す狙い (規範違反の除去 / 短縮 / 読みやすさ) を確認する
2. `references/writing-style.md` の各節を上から当て、外れた箇所を拾う。特に「LLM っぽい表現の禁止」「冗長の排除」「論証の厳密さ」
...
- `references/writing-style.md` の規範を SKILL.md 内に書き写す (規範は writing-style.md が正本)
