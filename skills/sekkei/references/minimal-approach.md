...
- 計画立案段階: `Minimal Approach:` で要求から直接読める規模との対比を必須化
- 推奨度: N/10 + 1 行根拠で「なんとなく良さそう」案の混入を抑止
- 3 案以上を提示しない: 選択肢過多の予防

## 書く前のラダー

コードを書く前に、上から順に下りる。止まれる段で止まる。止まれたら、それ以上書かない。

1. それは存在する必要があるか → ない: 作らない (YAGNI)
2. 標準ライブラリでできるか → できる: 使う
3. プラットフォーム標準機能でできるか → できる: 使う
4. 既に入っている依存でできるか → できる: 使う
5. 1 行で済むか → 済む: 1 行で書く
6. ここまでで止まれないときだけ、動く最小限を書く

### 怠惰だが手抜きではない

このラダーで削ってよいのは「無くても動くもの」だけ。次の 4 つは削らない:

- 信頼境界の検証 (外から来る値を信じない)
- データ損失の処理 (失敗時に書きかけを残さない)
- セキュリティ
- アクセシビリティ
