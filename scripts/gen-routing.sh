...
gen_routing() {
  local operation_rule
  operation_rule="$(sed -n 's/^- \(PRのマージと既定ブランチへの直接のpush.*\)$/\1/p' "$ROOT/scripts/contract.md")"
  [ -n "$operation_rule" ] || { echo "✘ shared operation rule is missing from scripts/contract.md" >&2; return 1; }
