...
| 全 skill 共通の契約 (tier / risk dial / invariant・procedure 定義 / handoff 形式 / 命名) を変える | 各 SKILL.md の `共通契約` block (5 skill で同一、`scripts/check-consistency.sh` が検査) |
...
| 利用先 project に注入する routing / tier 文を変える | `templates/CLAUDE.md` (注入 `session-context.sh` と `/hikizan:init` の単一ソース) |
...
- 破壊的操作や force push は、ユーザの明示確認なしに進めない (skill の共通契約 risk dial + hooks の floors で二重化)。
